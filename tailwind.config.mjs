/** @type {import('tailwindcss').Config} */
export default {
  content: ['./src/**/*.{astro,html,js,jsx,md,mdx,ts,tsx}'],
  theme: {
    extend: {
      colors: {
        vert: {
          DEFAULT: 'var(--color-vert, #1C6B35)',
          dark: 'var(--color-vert-dark, #14502A)',
          light: 'var(--color-vert-light, #2E8B4A)',
          50: '#F2F9F4',
          100: '#DCEFE1',
          200: '#B9DFC3',
          300: '#8FCB9F',
          400: '#5EB276',
          500: 'var(--color-vert-light, #2E8B4A)',
          600: 'var(--color-vert, #1C6B35)',
          700: 'var(--color-vert-dark, #14502A)',
          800: '#0E3B1F',
          900: '#092614',
        },
        rouge: {
          DEFAULT: 'var(--color-rouge, #8B1919)',
          dark: 'var(--color-rouge-dark, #6B0F0F)',
          light: 'var(--color-rouge-light, #B02525)',
          50: '#FCEEEE',
          100: '#F6D2D2',
          200: '#EDA5A5',
          300: '#E07878',
          400: '#CF4A4A',
          500: 'var(--color-rouge-light, #B02525)',
          600: 'var(--color-rouge, #8B1919)',
          700: 'var(--color-rouge-dark, #6B0F0F)',
          800: '#4A0808',
          900: '#2A0404',
        },
        accent: {
          DEFAULT: 'var(--accent, #1C6B35)',
          dark: 'var(--accent-dark, #14502A)',
          light: 'var(--accent-light, #2E8B4A)',
        },
        decor: {
          DEFAULT: 'var(--decor, #8B1919)',
          soft: 'var(--decor-soft, #F5C242)',
        },
        noir: {
          DEFAULT: '#0F0F0F',
          soft: '#1A1A1A',
          softer: '#2A2A2A',
        },
        texte: '#111111',
        indigo: {
          50:   '#f0f7f3',
          100:  '#d9eadf',
          200:  '#b6d5c2',
          300:  '#8cb89e',
          400:  '#5e9b75',
          500:  '#3b8254',
          600:  '#1C6B35',
          700:  '#155729',
          800:  '#0f4c21',
          900:  '#0a3a18',
          950:  '#062510',
        },
        amber: {
          50:    '#fcecec',
          100:   '#f7d2d2',
          200:   '#efa1a1',
          300:   '#e16e6e',
          400:   '#c93b3b',
          500:   '#8B1919',
          600:   '#781515',
          700:   '#621010',
          800:   '#4d0b0b',
          900:   '#380808',
          950:   '#200404',
        },
        emerald: {
          50:  '#ecfaf0',
          100: '#d2f1db',
          200: '#a8e2b9',
          300: '#79cf92',
          400: '#4eba6e',
          500: '#2ea355',
          600: '#2e8a48',
          700: '#266f3a',
          800: '#1f5830',
          900: '#194426',
          950: '#0d2916',
        },
        blue: {
          50:     '#f0f7f3',
          100:    '#d9eadf',
          200:    '#b6d5c2',
          300:    '#8cb89e',
          400:    '#5e9b75',
          500:    '#3b8254',
          600:    '#1C6B35',
          700:    '#155729',
          800:    '#0f4c21',
          900:    '#0a3a18',
          950:    '#062510',
        },
        slate: {
          50:    '#FFFFFF',
          100:   '#F6F6F4',
          200:   '#E8E8E5',
          300:   '#CECECA',
          400:   '#999996',
          500:   '#666663',
          600:   '#44443F',
          700:   '#2A2A26',
          800:   '#1A1A17',
          900:   '#0F0F0F',
          950:   '#050505',
        },
      },
      fontFamily: {
        // Typekit — loaded via <link rel="stylesheet" href="https://use.typekit.net/tci0qgy.css">
        pro: ['"filson-pro"', 'system-ui', 'sans-serif'],
        soft: ['"filson-soft"', 'system-ui', 'sans-serif'],
        sans: ['"filson-pro"', 'system-ui', 'sans-serif'],
        display: ['"filson-soft"', 'system-ui', 'sans-serif'],
      },
      fontSize: {
        // Display scale tuned for Filson Soft
        'display-sm': ['2.5rem', { lineHeight: '1.05', letterSpacing: '-0.01em', fontWeight: '700' }],
        'display-md': ['3.5rem', { lineHeight: '1.02', letterSpacing: '-0.02em', fontWeight: '700' }],
        'display-lg': ['4.5rem', { lineHeight: '1', letterSpacing: '-0.02em', fontWeight: '700' }],
        'display-xl': ['6rem', { lineHeight: '0.95', letterSpacing: '-0.03em', fontWeight: '700' }],
        'display-2xl': ['8rem', { lineHeight: '0.9', letterSpacing: '-0.03em', fontWeight: '700' }],
      },
      spacing: {
        section: '6rem',
        'section-sm': '4rem',
        'section-lg': '8rem',
      },
      boxShadow: {
        'logo': '0 2px 8px rgba(28, 107, 53, 0.08)',
        'logo-hover': '0 4px 16px rgba(28, 107, 53, 0.15)',
        'card': '0 1px 3px rgba(0, 0, 0, 0.05), 0 8px 24px rgba(0, 0, 0, 0.06)',
        'card-hover': '0 4px 12px rgba(0, 0, 0, 0.08), 0 16px 40px rgba(0, 0, 0, 0.1)',
      },
      maxWidth: {
        'content': '1760px',
        'prose-wide': '72ch',
      },
      animation: {
        'marquee': 'marquee 30s linear infinite',
        'fade-up': 'fadeUp 0.6s ease-out',
      },
      keyframes: {
        marquee: {
          '0%': { transform: 'translateX(0)' },
          '100%': { transform: 'translateX(-50%)' },
        },
        fadeUp: {
          '0%': { opacity: '0', transform: 'translateY(20px)' },
          '100%': { opacity: '1', transform: 'translateY(0)' },
        },
      },
    },
  },
  plugins: [],
};
