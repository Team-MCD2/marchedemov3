...
async function fetchText(url, attempt = 0) {
  try {
    ...
    if (attempt < 3) {
      await new Promise((s) => setTimeout(s, 1e3 * Math.pow(2, attempt)));
      return fetchText(url, attempt + 1);
    }
    return { status: 0, text: "", error: e.message };
  }
}
const decodeHtml = (s) => String(s ?? "").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&eacute;/g, "é").replace(/&egrave;/g, "è").replace(/&ecirc;/g, "ê").replace(/&agrave;/g, "à").replace(/&acirc;/g, "â").replace(/&ccedil;/g, "ç").replace(/&ocirc;/g, "ô").replace(/&ucirc;/g, "û").replace(/&icirc;/g, "î");
...
const POST = async ({ request, cookies }) => {
  ...
  try {
    ...
    for (const url of limitedUrls) {
      const { status: status2, text: text2 } = await fetchText(url);
      if (status2 === 200) {
        const product = parseProductPage(text2, url);
        if (product.nom) {
