import { g as guessRayonFromText } from '../../../../../chunks/inventaire-rayons_NoGWBJZi.mjs';
import { t as tryBarcodeLLMs } from '../../../../../chunks/inventaire-analyze_BjG_8-YC.mjs';
  ...
  try {
    ...
    if (llmResult && hasUsableLLMData(llmResult)) {
      ...
      return json({
        ...
        // 'llm:gemini' | 'llm:groq' | 'llm:mistral'
        confidence: "low",
    ...
    const reasonText = lookup.reason === "rate_limited" ? "Bases publiques temporairement indisponibles (limite de requêtes atteinte). Réessayez dans 1 minute, ou complétez manuellement." : llmError ? `Produit introuvable dans les bases publiques, et fallback IA indisponible (${llmError}). Complétez manuellement.` : "Produit introuvable (bases publiques + IA). Complétez les champs manuellement.";
    return json({
      result: emptyArticle(lookup.code_barres),
      source: lookup.reason,
      confidence: "none",
