...
Object.fromEntries(RAYONS.map((r) => [r.slug, r]));

/** Slugs valides (utilisé pour la validation côté API / formulaire). */
const RAYON_SLUGS = RAYONS.map((r) => r.slug);

/** Liste {slug, label} pour les <datalist> / <select>. */
const RAYON_OPTIONS = RAYONS.map(({ slug, label }) => ({ slug, label }));
 ...
 */
function guessRayonFromText(text) {
...
const MAGASIN_SLUGS = MAGASINS.map((m) => m.slug);

export { MAGASINS as M, RAYON_OPTIONS as R, MAGASIN_SLUGS as a, RAYON_SLUGS as b, RAYONS as c, guessRayonFromText as g };
