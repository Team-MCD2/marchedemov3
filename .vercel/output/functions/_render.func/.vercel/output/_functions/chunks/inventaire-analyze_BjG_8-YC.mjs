import { h as hasServerKey, a as hasGroqKey, b as hasMistralKey, c as hasVisionKey, v as visionAnnotate, e as extractFromVision, d as analyzeImage, f as analyzeImageGroq, g as analyzeImageMistral, i as analyzeBarcode, j as analyzeBarcodeGroq, k as analyzeBarcodeMistral } from './inventaire-vision_BLzAmBhu.mjs';
...
async function tryVisionLLMs({ base64DataUrl, geminiKey, model }) {
  const chain = [];
  if (geminiKey || hasServerKey()) {
    chain.push({ name: 'gemini', fn: () => analyzeImage({ base64DataUrl, apiKey: geminiKey, model }) });
  }
  if (hasGroqKey()) {
 ...
 */
async function analyzeImageHybrid({ base64DataUrl, geminiKey, visionKey, model }) {
  const useLLM = !!(geminiKey || hasServerKey() || hasGroqKey() || hasMistralKey());
 ...
 */
async function tryBarcodeLLMs({ barcode }) {
  const code = String(barcode || '').trim();
  if (!code) throw new Error('Code-barres manquant');

  const chain = [];
  if (hasServerKey()) chain.push({ name: 'gemini',  fn: () => analyzeBarcode({ barcode: code }) });
...
export { analyzeImageHybrid as a, tryBarcodeLLMs as t };
