...
import { a as MAGASIN_SLUGS, b as RAYON_SLUGS } from './inventaire-rayons_NoGWBJZi.mjs';
...
function getDb() {
  ...
  const key = (readEnv("INVENTAIRE_SUPABASE_SERVICE_ROLE_KEY") || readEnv("INVENTAIRE_SUPABASE_KEY")).trim();
  if (!url || !key) {
    throw new Error(
      "Supabase Inventaire non configuré. Définissez INVENTAIRE_SUPABASE_URL et INVENTAIRE_SUPABASE_SERVICE_ROLE_KEY dans .env.local."
    );
  }
...
function normalize(data, existing = null) {
  const qInit = data.quantite_initiale !== void 0 && data.quantite_initiale !== "" ? Number(data.quantite_initiale) : existing?.quantite_initiale ?? 0;
  const q = data.quantite !== void 0 && data.quantite !== "" ? Number(data.quantite) : existing?.quantite ?? qInit;
  ...
  const rawDlc = data.dlc !== void 0 ? data.dlc : existing?.dlc;
  let dlc = null;
  if (rawDlc) {
...
function rethrow(prefix, error) {
  ...
  if (code === "42P01" || code === "PGRST205" || /could not find the table/i.test(msg)) {
    friendly = "La table 'public.articles' est introuvable dans Supabase Inventaire. Veuillez vérifier vos credentials d'accès et votre base.";
  } else if (code === "42501" || /permission denied/i.test(msg)) {
    friendly = "Permission refusée sur la table 'articles'. Vérifie que INVENTAIRE_SUPABASE_SERVICE_ROLE_KEY est bien la clé 'service_role' (pas 'anon') et que la RLS n'a pas de policy bloquante.";
    ...
    friendly = "Clé Supabase Inventaire invalide. Vérifie INVENTAIRE_SUPABASE_SERVICE_ROLE_KEY dans .env.local.";
  } else if (/fetch failed|ENOTFOUND|ECONNREFUSED/i.test(msg)) {
    friendly = "Impossible de joindre Supabase. Vérifie INVENTAIRE_SUPABASE_URL et ta connexion.";
  }
  const err = new Error(`${prefix} : ${friendly}`);
  err.cause = error;
  throw err;
}
async function listArticles({ lite = false } = {}) {
  const db = getDb();
  const selectFields = lite ? "id,numero_article,nom_produit,marque,rayon,format,dlc,magasin,prix_vente,quantite,seuil_stock_faible,created_at,updated_at" : "*";
  const { data, error } = await db.from(TABLE).select(selectFields).order("created_at", { ascending: false });
...
async function nextNumArticle() {
  ...
  const db = getDb();
  const { data, error } = await db.from(TABLE).select("numero_article");
...
async function createArticle(data) {
  ...
  const { data: inserted, error } = await db.from(TABLE).insert({
    id,
    numero_article,
    ...fields,
    created_at: now,
...
async function createArticles(list) {
  const db = getDb();
  const now = Date.now();
  ...
  for (const item of list) {
    ...
    if (!numero_article) {
      ...
      if (!found) numero_article = prefix + Date.now() + Math.floor(Math.random() * 1e3);
    }
    payloads.push({
      id,
      numero_article,
...
async function updateArticle(id, data) {
  const db = getDb();
  const { data: existing, error: readErr } = await db.from(TABLE).select("*").eq("id", id).maybeSingle();
...
async function getStatsBundle({ topLimit = 10 } = {}) {
  const db = getDb();
  const { data, error } = await db.from(TABLE).select("id,numero_article,nom_produit,marque,rayon,format,dlc,magasin,prix_vente,quantite,seuil_stock_faible,created_at,updated_at");
  ...
  const by_rayon = [...byCat.entries()].map(([name, v]) => ({ name, ...v })).sort((a, b) => b.value - a.value);
  const topLight = topCandidates.sort((a, b) => b._value - a._value).slice(0, Math.max(0, topLimit));
  let top = topLight;
  if (topLight.length) {
    try {
      const ids = topLight.map((a) => a.id);
      const { data: photos, error: e2 } = await db.from(TABLE).select("id,photo_url").in("id", ids);
      if (!e2 && photos) {
...
async function findArticleByCode(q) {
  const query = String(q || "").trim();
  if (!query) return { found: false, article: null, articles: [] };
  const db = getDb();
  const { data: byNumData } = await db.from(TABLE).select("*").eq("numero_article", query).limit(1);
  if (byNumData?.[0]) {
    const article = decorate(byNumData[0]);
    return { found: true, match: "exact", article, articles: [article] };
  }
  const { data: byBarData } = await db.from(TABLE).select("*").eq("code_barres", query);
  ...
  const { data: lite, error } = await db.from(TABLE).select("id,numero_article,code_barres");
  ...
  if (matchedByNum) {
    ...
    return article ? { found: true, match: "partial", article, articles: [article] } : { found: false, article: null, articles: [] };
  }
  return { found: false, article: null, articles: [] };
}
async function getSyncStatus() {
  const db = getDb();
  const { data, error, count } = await db.from(TABLE).select("updated_at", { count: "exact", head: false }).order("updated_at", { ascending: false }).limit(1);
...
export { clearAllArticles, computeStatut, createArticle, createArticles, deleteArticle, findArticleByCode, getArticle, getStatsBundle, getSyncStatus, listArticles, nextNumArticle, updateArticle };
